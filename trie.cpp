...
void insert(TrieNode* root, string word)
{
    ...
    for (int i=0; i<l; i++)
    {
        ...
        //cout << word[i] <<" "<<cur<<endl;
        cur = cur->children[word[i] - 'a'];
    }
    cur->is_endofword = true;
}

bool search_string(TrieNode* &cur, string word)
{
    int l = word.length();
    for (int i=0; i<l; i++)
    {
...
{
    TrieNode* cur = root;
    if (!search_string(cur, word)) return false; 
    return cur && cur->is_endofword;
}

bool search_prefix(TrieNode* root, string prefix)
{
    TrieNode* cur = root;
    return search_string(cur, prefix);
}
